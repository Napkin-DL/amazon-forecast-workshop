...
def wait(callback, time_interval=30):
    ...
    print('')
    logging.info(
        f"Finished in {elapsed_time} seconds with status {job_status}")
...
def get_or_create_role_arn():
    ...
    # TODO: Only attach policies if not already attached
    ...
    # wait for 30sec to allow IAM role policy attachment to propagate
    time.sleep(30)
    print(role_arn)
    return role_arn
...
def plot_forecasts(fcsts, exact, freq='1H', forecastHorizon=24, time_back=80):
    ...
    plt.plot(time_int[-time_back:],
             exact['target'].values[-time_back:], color='r')
    plt.plot(pred_int, p50['Value'].values, color='k')
    plt.fill_between(p50['Timestamp'].values,
                     p10['Value'].values,
                     p90['Value'].values,
                     color='b', alpha=0.3)
    plt.axvline(x=pd.Timestamp(fcst_start_date),
                linewidth=3, color='g', ls='dashed')
    plt.axvline(x=pd.Timestamp(fcst_start_date, freq) +
                forecastHorizon-1, linewidth=3, color='g', ls='dashed')
